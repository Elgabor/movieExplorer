export function renderMovies(movies, favorites) {
    const resultsDiv = document.getElementById("result");
    resultsDiv.innerHTML = '';

    movies.forEach(element => {
        const isFavorite = favorites.includes(element.imdbID);
        const starClass = isFavorite ? 'text-yellow-400' : 'text-gray-300';
        ...
        const card = `
            <div class="max-w-xs bg-white rounded-xl shadow-lg overflow-hidden m-4 transform hover:scale-105 transition duration-300 cursor-pointer group">
                ...
                <div class="p-5 bg-white">
                    ...
                    class="favorite-btn absolute bottom-4 right-4 p-2 bg-white rounded-full shadow-md transform transition duration-200 focus:outline-none hover:scale-110"
                    >
                        <!-- Heroicon Solid Star -->
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ${starClass} transition-colors duration-200" fill="currentColor" viewBox="0 0 20 20">
